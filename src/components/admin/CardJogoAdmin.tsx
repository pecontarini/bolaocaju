...
import { STATUS_LABEL, statusBadgeClass } from "@/lib/admin/jogo-helpers";
...
export function CardJogoAdmin({ jogo, palpites, mostrarPalpites = false }: Props) {
  const aoVivo =
    jogo.status === "ativo" ||
    (jogo.placar_a !== null && jogo.placar_b !== null && jogo.status !== "encerrado");
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col items-end justify-center gap-1 pr-3 pl-1 py-2">
          ...
          {mostrarPalpites && (
            <span className="badge-palpite tabular-nums">
              {palpites ?? "—"} palpites
            </span>
          )}
